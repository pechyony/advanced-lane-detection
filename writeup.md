...
The pipeline for test images is managed by the code in cell 14 of `Advanced Lane Lines.ipynb` notebook.
...
The code for my perspective transform is in cells 8 and 9 of `Advanced Lane Lines.ipynb` notebook. I chose the following values of the source and destination points:
...
1. **Find base points of left and right lines**.  I computed a histogram of white pixels in the bottom strip of the image. This strip has height 200 and the same width as the gray scale image. In this histogram I counted the number of white pixels that have the same x value. Then I split the histogram into the left and right parts. The x value with the largest histogram value in the left part is the base of the left line. Similarly, the x value with the largest histogram value in the right part is the base of the right line. The code for this step is in the function `find_lane_base()` in cell 12.
...
The code for steps 2 and 3 is in the function `track_lane()` in cell 12. The functions `find_lane_base()` and `track_lane()` are called from the function `find_line()` (cell 13), which is the main function for finding and visualizing lanes.
...
The computation of the position of the vehicle with respect to the center is done in function `find_line()` in cell 13 and consists of the following two steps:
